...
class AbilityRegister:
    ...
    def list_non_planning_abilities_values(self , is_str = False) -> List[str]:
        output = []
        for ability in self.abilities.values():
            if ability.name in ("planner","is_task_done"):
                continue

            if is_str:
                output.append(str(ability))
            else:
                output.append(ability)
        return output

    def list_non_planning_abilities_names(self) -> List[str]:
        return [str(ability.name) for ability in self.list_non_planning_abilities_values()]

    def list_non_planning_abilities_name_description(self) -> List[str]:
        return "\n".join([f"{str(ability.name)}: {str(ability.description)}" for ability in self.list_non_planning_abilities_values()])
