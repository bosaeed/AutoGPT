...
class PreviousActionModel(Base):
    __tablename__ = "previous_action"
    action_id = Column(String, primary_key=True, index=True)
    task_id = Column(String)
    name = Column(String)
    args = Column(String)
...
class ForgeDatabase(AgentDB):
    ...
    async def add_previous_action(self, task_id,  output, name="", args=""):
        try:
            with self.Session() as session:
                new_action = PreviousActionModel(
                    action_id=str(uuid.uuid4()),
                    task_id=task_id,
                    name=name,
                    args=str(args),
                ...
                session.refresh(new_action)
                if self.debug_enabled:
                    LOG.debug(f"Created new Previous Action with task_id: {new_action.action_id}")
                return new_action
        except SQLAlchemyError as e:
            LOG.error(f"SQLAlchemy error while adding action: {e}")
            raise
        except NotFoundError as e:
            raise
        except Exception as e:
            LOG.error(f"Unexpected error while adding action: {e}")
            raise

    async def get_previous_action_history(self, task_id):
        if self.debug_enabled:
            LOG.debug(f"Getting previous action history with task_id: {task_id}")
        try:
            with self.Session() as session:
                if actions := (
                    session.query(PreviousActionModel)
                    .filter(PreviousActionModel.task_id == task_id)
                    .order_by(PreviousActionModel.created_at)
                    .all()
                ):
                    return [ a.output for a in actions]

                else:
                    LOG.error(
                        f"Previous Action history not found with task_id: {task_id}"
                    )
                    return []
                    # raise NotFoundError("Previous Action history not found")
        except SQLAlchemyError as e:
            LOG.error(f"SQLAlchemy error while getting Previous action history: {e}")
            raise
        except NotFoundError as e:
            raise
        except Exception as e:
            LOG.error(f"Unexpected error while getting Previous action history: {e}")
            raise